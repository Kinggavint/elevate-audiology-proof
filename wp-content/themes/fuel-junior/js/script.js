...
$(document).ready(function(){
	if($(window).width() <= 786) {
		...
		if(!$(".mobile-service-activator")[0]){
			service_setup();
			service_dynamic_size();
		}
	}
});
...
//ADD ADA to Service Buttons
$(document).click(function(){
	if($(".mobile-service-activator")[0]){
	var test = $(".mobile-service-activator").attr("class");
	if(test == "mobile-service-activator js-activate js-active"){
		$(".mobile-service-activator").attr( "aria-expanded","true");
	}else{
		$(".mobile-service-activator").attr("aria-expanded","false");
	}
}
});

function service_dynamic_size(){
	$('.service-content-wrapper').each(function(i){
		$(this).css("animation-name", "reset");
		let serviceHeight = $(this).height();
		$(this).removeAttr("style");
		$(this).css("--theHeight", serviceHeight+"px");
});
}

function service_setup(){
	$('.home-service-header').each(function(i){
		$(this).wrap('<button class="mobile-service-activator js-activate" aria-haspopup="true" aria-expanded="false" data-target="#service-item-'+ i+1 +'" id="mobile-service-activator-'+ i+1 +'"></button>');
});
$('.service-content-wrapper').each(function(i){
	$(this).attr({"id":"service-item-"+ i+1 +"", "data-target":"#service-item-"+ i+1 +""});
});
...
function service_taredown(){
	$('.home-service-header').each(function(){
		$(this).unwrap();
});
$('.service-content-wrapper').each(function(i){
	$(this).removeAttr("id data-target");
});
$('.services-container').each(function(){
...
window.addEventListener("DOMContentLoaded", function() {
		...
		observer = new IntersectionObserver(function(entries){
			for(let i=0;i<entries.length;i++){
				let entry = entries[i];
				var intersectionRatio = (entry.target.getAttribute('data-ratio') ? entry.target.getAttribute('data-ratio') : 0.1);
